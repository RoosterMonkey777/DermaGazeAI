...
class UserProfileManager {
    ...
    fun createUser(
        ...
        onSuccess: (List<String>) -> Unit,  // Now accepts a List<String> in the onSuccess callback
            ...
            { response ->
                if (response.hasErrors()) {
                    ...
                    response.data?.let { createdUser ->
                        triggerLambdaFunction(createdUser.id, onSuccess, onError)  // Now passes the callback expecting List<String>
                    ...
                    { updateResponse ->
                        if (updateResponse.hasErrors()) {
                            ...
                            updateResponse.data?.let { updatedUserProfile ->
                                triggerLambdaFunction(updatedUserProfile.id, onSuccess, onError)  // Now passes the callback expecting List<String>
    ...
    fun triggerLambdaFunction(
        userId: String,
        onSuccess: (List<String>) -> Unit, // Returning list of recommended product names
        onError: (Exception) -> Unit
    ) {
        val mutation = """
        ...
        Amplify.API.mutate(
            ...
            { response ->
                if (response.hasErrors()) {
                    ...
                    // Parse the response and extract the product names from the JSON structure
                    try {
                        val jsonObject = JSONObject(response.data)
                        val recommendations = jsonObject.getJSONArray("generateRecommendations")
                        val productNames = mutableListOf<String>()
                        for (i in 0 until recommendations.length()) {
                            productNames.add(recommendations.getString(i))
                        }

                        onSuccess(productNames)
                    } catch (e: JSONException) {
                        onError(Exception("Error parsing recommendations JSON: ${e.message}"))
                    }
                }
            },
            { error ->
